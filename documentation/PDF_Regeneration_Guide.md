...
| **User-configurable constants** (buffer sizes, ACK tuning, queue depth) and their defaults | `src/dcc/dcc_types.h` (validation) + the `dcc_user_config.h` templates (`templates/typical/`, `test/user_config/typical/`, and each app's own) | e.g. `USER_DEFINED_DCC_SCHEDULER_SLOT_COUNT`, `..._MAX_LOCOS`, `..._RAILCOM_BUFFER_DEPTH`, `..._SERVICE_MODE_RETRIES`, `..._ACK_THRESHOLD_MA`, `..._ACK_MIN/MAX_DURATION_US`, `..._DECODER_PACKET_QUEUE_DEPTH` |
