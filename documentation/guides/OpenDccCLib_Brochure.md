...
- **RailCom.** Command-station cutout timed by a five-state machine, armed at the end bit's last edge; 4/8 decode of Channel 1 and 2 replies, tagged with the packet's address. Decoder-side reply engine with bit-banged 4/8 transmit (the shipped decoder example leaves the transmit pin unwired).
- **Spec-correct repeats.** Every packet builder carries its repeat count, the standard's where it states one and a maintainer default where it is silent: two identical packets for a CV write, three for a date, one for a verify.
- **Dependency injection.** Hardware is reached only through function pointers wired at initialization. Port by rewriting the driver files for your timers, GPIO and UART; the protocol code is untouched.
...
| Module | Details |
|---|---|
| Bit encoder | NMRA 58 µs one-bit and 116 µs zero-bit halves (one and two ticks) from a shared 58 µs timer; single-buffered serialization from the ISR; RailCom cutout armed at the end bit's last edge |
...
| Packet decoder | Preamble ≥ 10, byte assembly, XOR check, matching for short and long addresses, and the CV 19 consist address for speed, direction and e-stop (accessory commands are delivered for the application to match), deferred dispatch queue so command callbacks never run in an interrupt |
...
Two ready-to-run projects ship with the library. Porting to a new platform means rewriting the driver files: timers and GPIO, UART, and for the decoder the ACK pulse.
...
- One `dcc_config_t` struct, one `DccConfig_initialize()` call; the command-station API comes from three small `dcc_application_command_station_*.h` headers.
...
- A single `DccConfig_run()` per main-loop pass drives the whole engine; every application callback fires there, except the decoder's `on_railcom_request`, which answers on the edge path.
- A shared 58 µs timer clocks both the main and the service track; the decoder's edge path classifies bits and assembles packets; the shipped example timestamps in the interrupt and drains in the main loop.
...
| Test cases | 1269, all passing, zero warnings |
| Coverage (gcovr) | 99.6 % lines, 100 % functions, 98.0 % branches |
