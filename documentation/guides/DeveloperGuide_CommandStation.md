...
At generation time: 28 test binaries, 1237 tests, 0 failures, 0 warnings; line coverage 99.5 %, function coverage 100 %, branch coverage 97.5 % (gcovr). The build also compiles six single-role configurations so a missing `DCC_COMPILE_*` guard fails as a compile or link error.
