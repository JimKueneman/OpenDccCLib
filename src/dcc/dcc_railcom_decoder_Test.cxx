...
static dcc_railcom_decoder_context_t test_ctx;
...
TEST(DccRailcomDecoder, initialize_does_not_crash) {
    ...
    DccRailcomDecoder_initialize(&test_ctx, &interface);
...
TEST(DccRailcomDecoder, initialize_clears_buffer) {
    ...
    EXPECT_EQ(DccRailcomDecoder_available(&test_ctx), (uint8_t)0);
...
TEST(DccRailcomDecoder, ch1_valid_2_bytes) {
    ...
    DccRailcomDecoder_begin_cutout(&test_ctx,42);
    DccRailcomDecoder_run(&test_ctx);
...
TEST(DccRailcomDecoder, ch1_invalid_byte_skipped) {
    ...
    load_uart_bytes(raw, 2);
    DccRailcomDecoder_begin_cutout(&test_ctx,10);
    DccRailcomDecoder_run(&test_ctx);
...
TEST(DccRailcomDecoder, ch2_valid_4_bytes) {
    ...
    DccRailcomDecoder_begin_cutout(&test_ctx,100);
    DccRailcomDecoder_run(&test_ctx);
...
TEST(DccRailcomDecoder, ch2_valid_2_bytes_minimum) {
    ...
    DccRailcomDecoder_begin_cutout(&test_ctx,200);
    DccRailcomDecoder_run(&test_ctx);
    ...
    EXPECT_EQ(DccRailcomDecoder_available(&test_ctx), (uint8_t)2);
...
TEST(DccRailcomDecoder, read_returns_buffered_datagram) {
    ...
    EXPECT_EQ(DccRailcomDecoder_available(&test_ctx), (uint8_t)1);

    dcc_railcom_datagram_t read_datagram;
    EXPECT_TRUE(DccRailcomDecoder_read(&test_ctx, &read_datagram));
...
TEST(DccRailcomDecoder, read_empty_returns_false) {
    ...
    dcc_railcom_datagram_t read_datagram;
    EXPECT_FALSE(DccRailcomDecoder_read(&test_ctx, &read_datagram));
...
TEST(DccRailcomDecoder, run_with_null_uart_does_nothing) {
    ...
    DccRailcomDecoder_begin_cutout(&test_ctx,1);
    DccRailcomDecoder_run(&test_ctx);
...
TEST(DccRailcomDecoder, cutout_address_tagged_correctly) {
    ...
    DccRailcomDecoder_begin_cutout(&test_ctx,9999);
    DccRailcomDecoder_run(&test_ctx);
...
TEST(DccRailcomDecoder, end_cutout_does_not_crash) {
    ...
    DccRailcomDecoder_end_cutout(&test_ctx);
...
TEST(DccRailcomDecoder, buffer_overflow_drops_oldest) {
    ...
    for (cutout_index = 0; cutout_index < 5; cutout_index++) {
        ...
        load_uart_bytes(raw, 2);
        DccRailcomDecoder_begin_cutout(&test_ctx,cutout_index + 1);
        DccRailcomDecoder_run(&test_ctx);

    }

    EXPECT_EQ(datagram_callback_count, (uint32_t)5);
    EXPECT_EQ(DccRailcomDecoder_available(&test_ctx), (uint8_t)4);
