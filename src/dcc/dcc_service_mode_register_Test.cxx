...
// Jim Kueneman's review of upstream PR #2 (2026-09-23): confirmed this module already resets
// register_state to IDLE on a failed preset/command start (unlike paged's equivalent, which did
// not -- see dcc_service_mode_paged_Test.cxx). Untested either way; pins it down.
TEST(DccServiceModeRegister, write_preset_begin_operation_fails_resets_to_idle) {
    ...
    /* A following call must be accepted, not rejected by a stuck register_state. */
    begin_operation_fail_on_call = 0;
    EXPECT_TRUE(DccServiceModeRegister_write(&test_context, 1, 0x55));
    EXPECT_EQ(begin_operation_count, (uint32_t)2);
...
TEST(DccServiceModeRegister, write_command_begin_operation_fails_completes_busy) {
    ...
    begin_operation_fail_on_call = 2;  /* the register command call fails to start */
