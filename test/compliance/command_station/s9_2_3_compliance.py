...
-- the exact risk the shared-ISR design carries (see ComplianceOverview.md).
...
def test_non_direct_bit_rmw(rep, s):
    """Non-Direct (register) bit ops are a byte-level READ-MODIFY-WRITE, not a native
    bit instruction (S-9.2.3 §E). With the register-aware mock decoder holding a known
    byte value, SVC REG BITW reads the byte (register-verify SCAN, ACKed early at the
    held value), modifies the target bit, and writes the byte back. On ch3 this shows
    as the verify scan (READ) followed by a register WRITE of the modified byte -- and
    no native Direct bit-manipulation packet. Without the value-match ACK the scan is a
    blind 0..255 (~90 s) walk, which is why this needs the mock."""
    clause = SPEC_DOC + " sec E (non-Direct bit op = read-modify-write)"
    HELD = 2                       # mock holds the CV byte = 2
    MODIFIED = HELD | (1 << 2)     # SVC REG BITW ... bit 2 = 1  ->  2 | 4 = 6
    _send_ok(s, f"SVC MOCKCV 1 {HELD}")

    # The scan retries per value, so the op is long -- capture wide.
    svc, result = _svc_capture_service(s, "SVC REG BITW 1 2 1", seconds=12.0)
    pkts = [list(d) for _, d in svc["packets"]]

    def is_reg_verify(p):  return len(p) == 3 and 0x70 <= p[0] <= 0x77
    def is_reg_write(p):   return len(p) == 3 and 0x78 <= p[0] <= 0x7F and p[0] != 0x7D

    verify_vals = sorted({p[1] for p in pkts if is_reg_verify(p)})
    rep.check(clause, "byte READ scan on the wire (register verify over multiple values)",
              len(verify_vals) >= 3,
              f"distinct register-verify values seen: {verify_vals} (the 0..{HELD}+ read scan)")

    first_verify = next((i for i, p in enumerate(pkts) if is_reg_verify(p)), None)
    write_idx = [i for i, p in enumerate(pkts) if is_reg_write(p) and p[1] == MODIFIED]
    rep.check(clause, f"modified byte written back (register write of 0x{MODIFIED:02X}) after the read",
              bool(write_idx) and first_verify is not None and write_idx[0] > first_verify,
              f"register-write(0x{MODIFIED:02X}) at index {write_idx[:1]}; first verify @ {first_verify}")

    has_direct_bit = any(len(p) == 4 and 0x78 <= p[0] <= 0x7B for p in pkts)
    rep.check(clause, "no native Direct bit-manipulation packet (byte-level RMW, not a bit op)",
              not has_direct_bit,
              "register mode reads+writes the whole byte rather than the Direct 4-byte bit instruction")
    _send_ok(s, "SVC MOCKCV OFF")
    time.sleep(0.2)
...
def test_register_cv_mapping(rep, s):
    """Register mode maps a CV to one of 8 physical registers via separate Mobile and
    Accessory tables (S-9.2.3 §E). The mapping RESULT is on the wire: every register
    command byte is 0111 C(reg-1), so the register field of the command on ch3 equals
    the mapped register. Sweep the mapping table and confirm each CV lands on its
    register (no decoder needed -- the command bytes appear regardless of ACK)."""
    clause = SPEC_DOC + " sec E (register-to-CV mapping)"
    MOBILE = [(1, 1), (2, 2), (3, 3), (4, 4), (29, 5), (7, 7), (8, 8)]
    ACCESS = [(513, 1), (7, 7), (520, 8)]

    def reg_of(p):     return (p[0] & 0x07) + 1
    def is_reg_cmd(p): return len(p) == 3 and 0x70 <= p[0] <= 0x7F and p[0] != 0x7D  # excl 0x7D page-preset

    def one(cv, dt, exp, label):
        svc, _ = _svc_capture_service(s, f"SVC REG WRITE {cv} 5 {dt}", seconds=3.0)
        pkts = [list(d) for _, d in svc["packets"]]
        regs = sorted({reg_of(p) for p in pkts if is_reg_cmd(p)})
        rep.check(clause, f"{label}: CV{cv} maps to register {exp}",
                  regs == [exp],
                  f"register command(s) on ch3 used register(s) {regs} (expected [{exp}])")

    for cv, exp in MOBILE:
        one(cv, "MOBILE", exp, "mobile")
    for cv, exp in ACCESS:
        one(cv, "ACC", exp, "accessory")
...
def run():
    ...
    try:
        ...
        # CS-023 needs the register/paged-aware mock decoder in the firmware (it ACKs
        # the byte-verify scan early). Without it, SVC REG BITW blind-scans 0..255
        # (~90 s, no abort) and jams the service-mode singleton -- keep it last.
        test_non_direct_bit_rmw(rep, s)
